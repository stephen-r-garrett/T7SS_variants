#!/bin/bash
echo Analysing 41 T7SSd loci using a DNA hmm profile built with hmmbuild to identify Ubiquitin-like EsaB homologues.
...
#locate tsdB gene sequences in the current strain
hmmbuild tsdB_search.hmm esaB_aln.fasta
...
#locate tsdB gene sequences in the current strain
nhmmer --tblout ${strain[index]}_tsdB.tblout TsdB_search.hmm ${assembly[index]}

#extract all tsdB-like sequences with an E-value < 5.0e-2 (0.05)
grep -v "^#" ${strain[index]}_tsdB.tblout | awk '{if ($13 < 5.0e-2){print $1"/"$9"-"$10, $9, $10, $1}}' | esl-sfetch -Cf ${assembly[index]} - > ${strain[index]}_tsdB.fa

#add in turn each tsdB-like sequence to a fasta file
cat ${strain[index]}_tsdB.fa >> tsdB_all.fa
